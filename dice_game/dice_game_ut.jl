...
listtuples_bundles = get_list_int_tuples_bundles()
listinteger_bundles = get_listinteger_bundles()

# Libraries
lib_float = Library(float_bundles)
lib_listtuples = Library(listtuples_bundles)
lib_listinteger = Library(listinteger_bundles)

# MetaLibrary
ml = MetaLibrary([lib_float, lib_listtuples, lib_listinteger])

### Model Architecture ###
model_arch = modelArchitecture(
    [Int, Int, Float64, Float64],
    [4, 4, 1, 1],
    [Float64, Vector{Tuple{Int,Int}}, Vector{Int}],
    [Float64],
    [1]
)
