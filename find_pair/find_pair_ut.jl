...
extra_inputs = [-1, 0, 1, 2]
# TRAIN DATA --- ---- 
X, Y = make_rows(
    ["input1", "input2"],
    [listinteger_caster, integer_caster],
    ["output1"],
...
listtutple_bundles = get_list_int_tuples_bundles()

# Libraries
lib_integer = Library(integer_bundles)
lib_listinteger = Library(listinteger_bundles)
lib_listtuple = Library(listtutple_bundles)

# MetaLibrary
ml = MetaLibrary([lib_integer, lib_listinteger, lib_listtuple])

### Model Architecture ###
model_arch = modelArchitecture(
    [Vector{Int}, Int, Int, Int, Int, Int],
    [2, 1, 1, 1, 1, 1],
    [Int, Vector{Int}, Vector{Tuple{Int,Int}}],
    [Int, Int],
    [1, 1]
)
